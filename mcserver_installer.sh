...
#Description	: A powerful bash script for easy installation of a Minecraft server (Vanilla, Forge, Spigot, Paper, Leaf & Arclight)                                                                                                                                                                   
...
ARCLIGHT_API_BASE="https://files.hypoglycemia.icu/v1/files/arclight"
...
arclight_script_creator() {
    local req=$1
    script_creator "$req" "screen -S Minecraft java -Xmx${ram_third}G -Xms512M -jar server.jar nogui"
}

arclight_script_creator_8()  { arclight_script_creator 8;  }

arclight_script_creator_16() { arclight_script_creator 16; }

arclight_script_creator_17() { arclight_script_creator 17; }

arclight_script_creator_21() { arclight_script_creator 21; }
...
folder_creator_arclight()    { folder_creator "Arclight-${version}_${arclight_loader}_Build-$build"; }
...
forge_vp_26()   { version_picker ver forge_custom_version 26.1 26.1.1 26.1.2; }
...
spigot_vp_26() { version_picker ver svp_26_callback 26.1 26.1.1 26.1.2; }
...
paper_vp_26()   { version_picker version "pvp_start 25" 26.1.1 26.1.2 26.2; }
...
############################################
## ARCLIGHT HELPER FUNCTIONS
############################################
## Start of Arclight Functions

arclight_loader_label() {
    case "$1" in
        forge) echo "Forge" ;;
...
arclight_ram_selector() {

    if [[ $version = "1.20.5" ]] || [[ $version = "1.20.6" ]] || [[ $version = "1.21"* ]]
    then
        select_ram arclight_script_creator_21
    elif [[ $version = "1.17"* ]]
    then
        select_ram arclight_script_creator_16
    elif [[ $version = "1.18"* ]] || [[ $version = "1.19"* ]] || [[ $version = "1.20"* ]]
    then
        select_ram arclight_script_creator_17
    else
        select_ram arclight_script_creator_8
    fi
...
arclight_download_jar() {

    folder_creator_arclight
    cd "$path/Servers/$dirname"
    wget "$arclight_download_url" -O server.jar
    arclight_ram_selector

}


arclight_build_select() {

    curl -L -s "$ARCLIGHT_API_BASE/minecraft/$version/loaders/$arclight_loader/versions-snapshot" > "$path/arclight_builds.json"

    if ! jq -e '.files | length > 0' "$path/arclight_builds.json" >/dev/null 2>&1
    then
        rm -f "$path/arclight_builds.json"
        dialog --msgbox "No Arclight builds found for Minecraft $version on $(arclight_loader_label "$arclight_loader")." 8 70
        clear
        return
    fi

    menu_items=()
    while IFS=$'\t' read -r build_name build_date
    do
        if [[ $build_name == *"-SNAPSHOT"* ]]
        then
            menu_items+=("$build_name" "$build_date snapshot")
        else
            menu_items+=("$build_name" "$build_date stable")
        fi
    done < <(jq -r '.files | sort_by(."last-modified") | reverse[] | [.name, (."last-modified" | split("T")[0])] | @tsv' "$path/arclight_builds.json")

    build=$(dialog --clear \
                  --backtitle "MC-Server Installer by realTM" \
                  --title "Select Arclight Build" \
                  --menu "Select the Arclight build you want to install:" \
                  40 100 20 \
                  "${menu_items[@]}" \
                  2>&1 >/dev/tty)
    ...
    arclight_download_url=$(jq -r --arg build "$build" '.files[] | select(.name == $build) | .link' "$path/arclight_builds.json")
    rm -f "$path/arclight_builds.json"

    if [[ -z "$arclight_download_url" ]] || [[ "$arclight_download_url" == "null" ]]
    then
        dialog --msgbox "Unable to find a download URL for the selected Arclight build." 7 70
        clear
        return
    fi
...
arclight_loader_select() {

    curl -L -s "$ARCLIGHT_API_BASE/minecraft/$version/loaders" > "$path/arclight_loaders.json"

    if ! jq -e '.files | length > 0' "$path/arclight_loaders.json" >/dev/null 2>&1
    then
        rm -f "$path/arclight_loaders.json"
        dialog --msgbox "No Arclight loaders found for Minecraft $version." 7 60
        clear
        return
    fi

    menu_items=()
    while read -r loader_name
    do
        menu_items+=("$loader_name" "$(arclight_loader_label "$loader_name")")
    done < <(jq -r '.files[].name' "$path/arclight_loaders.json")
    rm -f "$path/arclight_loaders.json"

    arclight_loader=$(dialog --clear \
                  --backtitle "MC-Server Installer by realTM" \
                  --title "Select Arclight Loader" \
                  --menu "Select the modloader for Arclight:" \
                  20 60 10 \
                  "${menu_items[@]}" \
                  2>&1 >/dev/tty)
...
arclight_version_select() {

    curl -L -s "$ARCLIGHT_API_BASE/minecraft" > "$path/arclight_versions.json"

    if ! jq -e '.files | length > 0' "$path/arclight_versions.json" >/dev/null 2>&1
    then
        rm -f "$path/arclight_versions.json"
        dialog --msgbox "Unable to load Arclight Minecraft versions." 7 60
        clear
        return
    fi

    menu_items=()
    while read -r arclight_version
    do
        menu_items+=("$arclight_version" "")
    done < <(jq -r '.files[].name' "$path/arclight_versions.json" | sort -V -r)
    rm -f "$path/arclight_versions.json"

    version=$(dialog --clear \
                  --backtitle "MC-Server Installer by realTM" \
                  --title "Select Arclight Version" \
                  --menu "Select the Minecraft version for Arclight:" \
                  30 70 15 \
                  "${menu_items[@]}" \
                  2>&1 >/dev/tty)
